#!/usr/bin/env bash
# ============================================================================
# seed-demo.sh — 데모 시드 재적재를 "한 줄"로 (기본 + 의사 사진 + 상담봇 데모까지 한 번에).
# ----------------------------------------------------------------------------
...
# 상담봇(4단계) 데모 데이터 — 문의 티켓함·상담봇 기록·안내자료(KB)·오답/미해결/품질 화면을 채운다.
# ⭐ 일부러 seed_demo.sql에 합치지 않고 **별도 psql(=별도 트랜잭션)**로 부른다: 챗봇 시드가
#    실패해도 위 기본 시드(직원·환자·예약)와 로그인은 무사하다. if 조건 안이라 set -e에도 안 죽는다.
echo "▶ 상담봇 데모 데이터 적재 중…"
if docker exec -e PGTZ=Asia/Seoul -i "$CONTAINER" \
     psql -U postgres -d postgres -v ON_ERROR_STOP=1 -q < "$DIR/seed_demo_chat.sql"; then
