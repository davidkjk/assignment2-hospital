...
-- ⭐ 세션 시간대를 KST로 고정한다. 이 시드의 `current_date`·`now()::date`는 "데모의 오늘"을
--    정하는데, 원격 재시딩은 세션 풀러(Supavisor)를 거친다. 풀러는 접속 startup 파라미터인
--    PGTZ(=seed-demo-remote.sh의 `PGTZ=Asia/Seoul`)를 **버려서** 세션이 UTC로 돈다
--    → current_date가 UTC 날짜가 되고, 밴쿠버 08시 재시딩(=UTC 15/16시=한국 다음날 00~01시)엔
--    UTC와 한국 날짜가 하루 어긋나 일요일 등 휴무일에 「미접수·시각경과」 유령 예약이 생긴다.
--    런타임 `SET TIME ZONE`은 startup 파라미터가 아니라 풀러를 통과하므로 여기서 못박는다
--    ([[reference-supavisor-drops-timezone-startup-param]]).
...
-- ⚠️ 이 시드는 `current_date`를 쓴다 — psql 세션 시간대에 걸린다. ~~PGTZ=Asia/Seoul로 넣어야
--    맞는다~~ ✅ **해소(2026-09-13)** — 원격 풀러가 PGTZ(startup)를 버려 UTC로 돌던 것을,
--    위 `begin;` 직후 런타임 `set time zone 'Asia/Seoul'`로 못박아 해결. PGTZ(스크립트)는
--    로컬 docker exec용으로 남겨두되, 원격은 이 런타임 SET이 정본이다.
