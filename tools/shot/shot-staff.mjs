...
// 첫 진료과(강수아) 카드 클릭
await p.evaluate(() => { const b=[...document.querySelectorAll('button')].find(x=>x.textContent.includes('강수아')); if(b) b.click() })
await s(1800)
console.log('at', p.url())
await p.screenshot({ path: `${process.env.S}/shot/${PREFIX}-staff.png`, fullPage: true })
await b.close()
