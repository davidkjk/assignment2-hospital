...
export function DoorProvider({ children }: { children: ReactNode }) {
  ...
  const api = useMemo<DoorApi>(() => {
    ...
    return {
      ...
      open: (door) => {
        ...
        setActiveField(door === 'reserve' ? 'patient' : null)
      },
      close: reset,
      finish: (text) => {
        setFlash({ door: openDoor ?? 'reserve', text })
        reset()
      },
      ...
      pickDoctor: (d) => {
        ...
        setActiveField(openDoor === 'reserve' ? 'time' : null)
      },
      pickSlot: (date, time) => {
  ...
  }, [openDoor, collapsed, activeField, draft, flash])
...
export function useDoors(): DoorApi {
  ...
  if (!v) throw new Error('useDoors must be used within DoorProvider')
  return v
}
