...
function MemberCard({ member, onEdit, onAppointment }: {
  ...
  const isSelf = member.relation === '본인'

  return (
    <Card data-testid={`family-member-${member.id}`} className="overflow-hidden">
      <div className="flex items-stretch">
        {/* 채운 딥틸 레일 = 카드의 솔리드 앵커. 관계를 담고 본인은 다른 색으로 구분(목업 fm-rail). */}
        <div
          className={`flex w-14 shrink-0 items-center justify-center px-1 text-center text-xs font-bold ${
            isSelf ? 'bg-indigo-100 text-indigo-700' : 'bg-primary text-primary-foreground'
          }`}
          aria-hidden="true"
        >
          {member.relation}
