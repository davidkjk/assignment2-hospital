...
def test_여섯_토글이_열_종류를_빠짐없이_덮는다():
    """[SET-NOTI-04] 2묶음·6토글. 지금 발송되는 모든 notification_type이 정확히 한 토글씩에 든다 —
    빠지면 그 알림은 끌 방법이 없고(SET-NOTI-01 위반), 겹치면 한 토글이 남의 알림을 끈다.
    ⚠️ questionnaire_partial(T24)·support_answered(챗봇)는 아직 MESSAGES에 없는 예정 종류라 초과분으로만 허용."""
    ...
    assert set(covered) - set(MESSAGES) == {"questionnaire_partial", "support_answered"}  # 초과=예정 2종뿐
...
@pytest.mark.asyncio
async def test_줄이_없으면_여섯_토글이_모두_켜짐(committed_conn):
