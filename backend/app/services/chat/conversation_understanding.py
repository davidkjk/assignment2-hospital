...
_CONSERVATIVE_ROUTING_PRINCIPLE = (
    "라우팅 판단 원칙(중요): 사용자가 실제로 예약·예약변경·예약취소·문진 작성 같은 '행동'을 해 달라고 "
    "명확히 요청할 때만 route를 agent로 정하세요. 방법·절차·준비물·발급 방법·가능 여부 등 '정보를 묻는 질문'은 "
    "행동 요청이 아니므로, 예약과 관련돼 보여도 agent가 아니라 rag입니다. 행동인지 정보 문의인지 애매하면 "
    "agent가 아니라 rag를 기본값으로 하세요."
)
