...
async def create_phone_appointment(
    ...
    async def _run(c) -> UUID:
        ...
        # ⛔ 숫자를 여기 박지 않는다 — hospital_settings.booking_window_weeks 하나가 슬롯 생성·
        #    문자 예약·이 검증을 함께 지배한다(SCHED-WINDOW-01 · 갭 #47 재발 방지).
        ...
        window_weeks = await settings_service.get_booking_window_weeks(c)
        if await c.fetchval(
            "select $1::timestamptz::date > (current_date + make_interval(weeks => $2))::date",
            start_at, window_weeks,
        ):
            raise AppError(
