...
def _provider_push(token: str, body: str) -> str:  # pragma: no cover - 배포에서 구현
    raise NotImplementedError("실제 FCM 푸시 전송은 배포(env)에서 주입한다.")


def _provider_sms(phone: str, body: str) -> SmsOutcome:  # pragma: no cover - 배포에서 구현
    raise NotImplementedError("실제 Twilio 문자 전송은 배포(env)에서 주입한다.")
...
async def _dispatch_one(nid, conn, push_send, sms_send) -> None:
    row = await conn.fetchrow(
        "select patient_id, requested_channel, channel, body from notification_log "
        "where id=$1 for update", nid)
    if row is None:
        return
    ...
    if wants_sms:
        if await _sms_eligible(conn, row["patient_id"]):
            outcome = sms_send(await _phone(conn, row["patient_id"]), body)
            if outcome.status == "queued":
                # 접수까지만 안다 — '발송중' 유지, 도달/실패는 콜백으로(SEND-RESULT-02).
                await conn.execute(
                    "update notification_log set channel='sms', provider_message_id=$2 "
                    "where id=$1", nid, outcome.provider_message_id)
            else:
