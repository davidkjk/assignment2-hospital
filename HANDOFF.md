...
**갭 17건 반영 재검증 완료(2026-07-30 밤)**: 위 커밋들의 diff를 전부 대조해 실제 반영 여부 확인. 발견된 문서 오류 4건 전부 수정·커밋:
- 5.2절 "9개 항목 전체" 과장 표현 → 7개(RAG)+2개(행동형 도구) 분리 명시. 커밋 `231d311`.
- staff-web-design.md 섹션 4.5의 "섹션 6 끝부분 참고" 혼란스러운 상호참조 수정. 커밋 `231d311`.
- staff-web.md Task 24: `ConfirmIdentityDialog` 재입력 흐름이라는 잘못된 주석을 실제 동작(마스킹 값 표시-확인)에 맞게 수정 + 죽은 코드 한 줄 제거. 커밋 `902712e`.
기능적 버그는 없었음 — 전부 문서 내 설명 문구가 실제 코드/다른 문단과 어긋나는 정합성 문제였음.
