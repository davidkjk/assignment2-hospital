...
2. ~~**디자인 원본 = 목업 98~103** — 홈페이지 내장 채팅창(가짜)이 아니라 공식 위젯 목업을 시각 정본으로 삼는다.~~ ✅ **변경(2026-09-05, 사용자 결정)** — **형태의 디자인 원본은 홈페이지 내장 챗봇 위젯 형태**(`.chat*`, 더 세련됨: 둥근·아바타·상태점·부드러운 그림자)로 바꾼다. 색은 양쪽 동일(`#0B6E70`)이라 형태만 이식. **상태·문구·규칙 정본은 여전히 목업 98~103 + `screen-behaviors.md`.** (가짜 채팅의 "동작"이 아니라 "형태"만 참고 — 로직은 webchat.) 역참조: `HANDOFF-chatbot.md` ⭐(2026-09-05), 계획서 Global Constraints, `widget.css` 헤더.
