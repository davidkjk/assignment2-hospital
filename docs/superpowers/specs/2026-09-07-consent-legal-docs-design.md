...
>
> ✅ **해소(2026-09-08, 브랜치 feat/consent-age-gate)** — **만 14세 미만 = 사용자 결정 Option A(앱 가입 차단 + 병원 안내)**. 기각: Option B(앱 내 법정대리인 동의 흐름 — 「휴대폰 소유 = 법정대리인 자격」 자동증명 안 돼 병원 법무검토 필요, 가상병원이라 채택 안 함). 이 결정으로 원설계 item #4(만14세 분기)·#5(서버 우회차단)·#6(아동 광고)이 함께 해소되고, **법정대리인 증적 스키마(#7의 법정대리인 부분)는 불요**(아동을 애초에 안 받으므로). 규칙=`screen-behaviors.md` `AGE-GATE-01~06`. 구현: 연령 게이트+차단 화면(`signup_age_gate.dart`)·서버 재계산(`age_gate.py`·`register_profile`).
