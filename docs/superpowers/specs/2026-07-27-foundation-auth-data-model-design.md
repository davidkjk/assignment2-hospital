...
- ⚠️ **재확정(rebook) 재발송 주의** (④ 마이그레이션 구현 중 확인, 2026-08-15): 1회성 종류의 dedup은 "예약당 한 번"이라, 예약을 바꿔 **다시 확정**하면 두 번째 확정 안내가 잠금에 걸려 **조용히 안 나간다.** 발송 로직(2·3단계)은 재확정을 **새 `appointments` 행 또는 다른 `notification_type`로 라우팅**해 재발송 누락을 막는다 — dedup 자물쇠 자체는 유지한다(#121과 충돌 아님).
- **환자 열람 RLS는 3단계**: `notification_log`는 foundation에서 직원 발송 이력 조회(`is_active_staff` select)만 연다. 환자 앱 알림함이 **본인 알림**을 읽는 정책은 **3단계(환자 인증 연동)**에서 추가한다(쓰기는 dispatcher가 서비스 역할로만).
...
- **환자 self 정책은 3단계** (④ 마이그레이션 구현 중 확인, 2026-08-15): foundation에서는 RLS만 켜고 정책을 두지 않는다(dispatcher가 서비스 역할로 접근). 환자 **본인 읽기/수정** 정책은 **3단계(환자 인증)**에서 붙으며, 그때까지 `insert/update` grant는 RLS 기본 거부로 무효다.
