...
- **매 메시지 처리 순서**(구조 결정 「3-A 실행 순서와 3갈래 체인」): **⓪ 응급 표현 검사(규칙 기반 키워드, AI 호출 전 코드가 직접) → ⓪-b 명시적 직원 연결 요청(규칙 기반 키워드, `staff_request`) → ① 인계 감시(6조건, 라우팅과 무관하게 매 턴) → ② 라우터(`RunnableBranch`, 3갈래)**. ⓪이면 119/응급실 안내 후 종료, ①이면 진행 중 갈래를 중단하고 인계 체인으로 강제 전환.
  - ⭐ **⓪-b 명시적 직원 연결 요청(2026-09-05 사용자 결정, 신설)**: 사용자가 "직원 연결/상담원 바꿔/사람이랑 연결" 등으로 **직접 사람을 찾으면** 응급처럼 **결정적(키워드)** 으로 바로 인계한다. 기존 6조건(unhelpful·no_answer·repeated·medical_judgment·data_mismatch·complaint)엔 "사용자가 직접 직원을 요청" 조건이 없어, 타이핑 요청이 `no_answer` 폴백으로만 겨우 닿던 갭을 메운다. 프론트 `[직원에게 문의]` 버튼(콜백)과 동등한 경로를 **타이핑에도** 연다. 큐레이션 = 연결 의도어("연결/바꿔/문의/물어") 동반 필수(단순 언급 "직원분 친절해요?"는 제외). 구현=`safety_watchdog.check_staff_request`·`orchestrator` ⓪-b, 사유=`staff_request`.
