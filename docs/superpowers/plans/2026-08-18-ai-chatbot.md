...
>
> ⚠️ **삭제된 상류 작업본 안내(2026-08-26)**: 태스크가 `근거 원본`으로 인용하는 `.claude/codex-work/orchestration/3A-schema-requirements-2026-08-13.md`(§4.1·§4.3·§4.6·§4.7·§6·§7·§9 등)는 2026-08-26에 삭제됐다. **그 파일의 내용은 결정로그 `docs/superpowers/specs/2026-07-31-ui-design-decisions.md`의 「상담봇 통합 스키마 — 상세 요구 (3-A, 2026-08-13)」 절에 통째로 병합돼 있다**(줄번호는 밀리니 이 제목으로 grep; 2026-08-26 기준 ~4604줄 시작, `### 4.1 chat_threads`·`chat_messages`·`chat_read_states`·`chat_notification_batches`·`NotificationRecipient` 컬럼표 포함, 공백 7건표는 「⑦플랜 대조 — 통합 스키마 공백 7건」 절). 즉 인용의 `3A §4.1`은 = 결정로그 그 절의 `4.1`과 일치. **이미 채워진 태스크(예: Task 1)는 컬럼·enum 허용값·기각 이유·인덱스·RLS·테스트를 전부 인라인으로 갖고 있어 그 파일 없이 구현 가능**하고, **미작성 태스크는 위 결정로그 절·spec-index를 근거로 채운다**. 삭제된 파일의 줄 앵커(§390-405 등)는 죽은 링크·역사적 출처 표시일 뿐이다.
