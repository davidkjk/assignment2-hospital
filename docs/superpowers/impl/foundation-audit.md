...
## 코디네이터 리뷰 결과 (2026-08-20 밤, ⑦ 첫 코드 패킷)

감사가 뽑은 3건을 코디네이터가 원문 대조로 리뷰한 결과:

| 항목 | 판정 | 처리 |
| --- | --- | --- |
| 인증 실패 계정 열거 방지 | 기반-clean, 적용 | ✅ 완료 (커밋 `f00ac79`, TDD `test_staff_auth_router.py`) |
| Python DB 예외 원문 노출 6곳 | 기반-clean, 적용 | ✅ 완료 (커밋 `e40a67f`, TDD `test_error_masking.py`) — 감사 처방(6곳 전부 고정 한글) 대신 **SQLSTATE 구분**으로 정정: 트리거·RPC의 한글 안내(P0클래스)는 그대로 노출, 예기치 못한 드라이버 오류만 로그+고정 문구(플랜 「친절한 안내는 서버, DB가 최종 심판」·기존 revise P0003→409 로직 보존) |
| 예약 상태 한 칸 역전이 | **재분류 → 직원웹 소유** | ⏭️ **이월(구현 안 함)**. 이건 갭 #82(`UNDO-IMPL-01/02/04`) = 직원웹 「공통 패널·확인창·되돌리기」 태스크가 마이그(`00037_status_undo.sql`)·`VALID_TRANSITIONS`·프론트 `UndoControl`을 한 덩어리로 소유. 마이그 번호도 미정(테스트 주석 `00021` vs 지시 `00037`)이라 C2 맥락에서 직원웹 태스크 때 확정. 백엔드 반쪽만 앞당기면 직원웹 태스크가 중복 생성·충돌. (사용자 확정 2026-08-20) |

**결론: foundation-owned 잔여 = 실제 2건(둘 다 완료). 감사가 3번째를 과다 포함했음.**
다음 세션은 잔여3을 기반 잔여로 재시도하지 말 것 — 직원웹 PANEL/UNDO 태스크에서 백엔드+프론트 함께.
