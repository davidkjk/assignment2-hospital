...
- **Ⓐ branchA_on**(env 3개 ON): _(대기 — 배포 후 `bot_probe --label branchA_on` → `--diff`)_. **핵심 검증**: #1·2·3이 `card→rag(text)`로 교정되나 + #6·8 예약 유지(과보정 없음) + 센티넬 영어 노출 소멸 + 안전 무회귀. CT류가 여전히 오라우팅/no_answer면 **2단계(조건부 재검색 루프) 근거**.
