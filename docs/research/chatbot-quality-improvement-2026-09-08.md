...
| 예약 취소하려고요 | `rag(안내문)` → **`no_answer/handoff`** | ⚠️ **약한 퇴행** — 도움 문구 사라지고 바로 인계 |
| 예약 시간 변경 | `card(agent)` → **`rag(안내문)`** | ◐ 경계선(빈 카드→"앱에서 변경" 실용 안내) |
| (그 외 9문항) | 무변화 | ✅ 무회귀 |

- **판정**: **핵심 표적(정보질문 오라우팅) 4/4 해결**. 안전 불변식(#14 진료과·#15 진단요구 인계), 진짜 예약(#6 agent 유지=과보정 아님), 검색 답(주차·CT금식·위내시경 등) **전부 무회귀**. 브랜치 A는 두 불만 중 "틀린 답"의 라우팅 갈래에 **명확한 순이익**.
- **⚠️ 후속(#7 취소)**: 보수 원칙이 "예약 취소하려고요"를 agent로 보냈고, 익명 세션은 취소 실행 불가 → `no_answer/handoff`로 떨어져 ⓪의 유용한 안내문(WEBCHAT-CANCEL-GUIDE)을 잃음. → 원칙을 **"취소·변경 '방법 문의'는 rag, 실제 실행 의도만 agent"**로 한 줄 좁히거나, agent 취소 경로가 익명일 때 안내문으로 폴백하게. #8 변경은 오히려 안내가 실용적이라 유지 가능.
- **⚠️ 비결정성 주의**: #7·#8은 단발 분류라 LLM 실행 편차 가능. 확정 전 재실행 1~2회 권장.
