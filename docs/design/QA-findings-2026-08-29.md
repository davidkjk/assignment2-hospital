...
- ✅ **L23 [전일 미완료 목록] 줄바꿈 — 해소(2026-08-29)**: 원인 — 시각 레일이 `w-14`(56px)인데 전일 행만 `8/29 09:30`(날짜+시각)이라 토큰 중간에서 못나게 접혔다. **수정**: rail을 **날짜(작게·위)+시각(굵게·아래) 두 줄**로 의도 정돈(`railDate` 필드 신설, `TODAY-YDAY-03`의 「날짜 함께 표시」 유지). 이름+정보 행도 레일 안정화로 한 줄에 들어옴. 검증: TodayPage 23/23·tsc·헤드리스 스샷(`today-l23.png`)에서 8/29+09:30 두 줄·정보 한 줄 확인.
