...
- 🟠 **L56 [안내 보내기] 한 화면 통합 + 받는사람 컨트롤 + 고른 사람 표시(결정 필요)**: ① 새로 만든 「새 안내 보내기」를 **첫 화면 상단**, 「안내발송 이력」 카드를 **하단**에 둬 **한 화면**으로. ② 그러면 「새 안내 보내기」 **제목은 제거** 가능. ③ 「받는 사람」 탭(세그먼트 버튼)의 **회색 배경이 버튼인데 쓸데없이 김** → 버튼 폭/스타일 정돈. ④ 검색·선택은 좋으나 **고른 사람들을 보여줄 영역이 없음** → 위치 결정 필요(⏳ 사용자 결정). ⚠️ L12·L13에서 본화면 2단으로 이미 옮긴 것과 연결 — 그 구조 위에서 재배치.
...
- 🟠 **L59 [의사 진료] 진료기록 작성 완료 → 확인 팝업 후 무동작(맞나? 결정)**: 진료기록 작성 완료 누르면 확인 팝업, 확인 후 **아무 일 안 일어나고 그 환자 페이지 그대로**. 완료 후 무엇이 일어나야 하나(목록에서 내려가기/다음 환자로/상태 전이) = ⏳ 확인·결정.
- 🟠 **L60 [의사 진료] 완료된 환자 되돌려 수정하는 경로(결정)**: 진료날짜 뒤로 가서 전날 보면 오늘 대기에 완료 안 누른 사람이 남음. 완료 누르면 다른 목록으로 내려감. **완료 후 다시 꺼내 진료기록 수정하는 화면이 없음** → 탭 추가? 메뉴? [완료 목록] 버튼+사이드패널? = ⏳ 사용자 결정(L59와 함께).
- 🔴 **L61 [오늘 진료 대기] 상태 체계 혼란**(스크린샷 `AL3DHg`): 같은 목록에 **「도착」과 「진료중」 배지가 섞여 나옴**(1·2번=도착, 3~6번=진료중), **진료중인데 「대기 98분」**으로 표시, 도착이 왜 1-2번인지 순서 불명. **여러 명이 동시에 「진료중」**은 로직상 이상(한 번에 한 명). → 시딩 상태 or 상태 전이/표시 로직 확인. 규칙(`QUEUE-*`·의사 콘솔 상태 모델) 대조 필요.
- 🔴 **L62 [사전문진 상세] raw JSON을 글자단위로 렌더**(스크린샷 `bJmuLW`): 사전문진 상세가 답변 대신 `"`, `,`, `q`, `3`, `:`, `아`,`니`,`오`, `}` 를 **한 글자씩 줄번호와 함께** 표시 = 답변 문자열/JSON을 배열처럼 렌더. 시딩 데이터 형식 문제 or 상세 렌더 버그. L40 문진 시드 형식 교체와 연관 가능. → 원인 확인.
- 🟡 **L63 [사전문진 패널] 빈 상태 UI 데모와 다름**(스크린샷 `rhCFSD`): 「제출된 사전문진이 없습니다」 빈 상태 패널이 데모와 다른 모양 → 데모 대조 후 정렬.
- 🟠 **L64 [데모 계정] 의사 계정 하나 로그인/작동 불가**: 데모 의사 계정 중 하나가 작동 안 함 → 어느 계정인지 특정 + 시드/auth 원인 확인. L26(doctor8 초대 대기)과 구분.
