...
- ✅ **L65 [의사 콘솔] 가운데 「환자 맥락」 열 데모정렬 — 해소(2026-09-01 밤 재작업)**: 부분완료(커밋 5f097c9)에서 남았던 3건을 real·demo를 같은 폭 카드 단위로 잘라 대조하며 마무리. ① **프레임 통합** — 열 프레임(패딩·gap·bg·borderRight)을 `DoctorConsolePage`의 `contextCol` 스타일로 올리고, `ContextPanel`에서 자기 패딩·bg·borderRight를 떼어 네 카드(기본정보·예약이유·사전문진·메모)가 한 열 안에서 동일 인셋·sp-3 간격이 되게(데모 `w-80 p-4 bg-muted/20` 한 열 구조와 일치). ② **기본정보 둘째 줄** = `09:45 · 내과`(예약 슬롯 start + 로그인 의사 진료과). 슬롯 시각은 `get_doctor_queue`에 `s.start_time` 추가로 배선, 진료과는 `/me`가 그동안 `department_name`을 안 내려 `staff.departmentName`이 늘 null이던 **잠복 갭**을 함께 해소(사이드바 부제도 이제 진료과 표시). ③ **배지 제거**(사용자 결정) — 진료대기/도착 배지는 여기선 불필요, 대기열에 이미 있고 홀로 놓이면 조회 오류처럼 보인다. + **부수 통일**: `NoteSection` 테두리 `2px`→`1px`(환자상세 다른 섹션·콘솔 카드와 통일, 이 컴포넌트만 2px였음). 검증: tsc0·doctor+patient 71 vitest·헤드리스 카드 단위 대조(`l65-real-top.png` ↔ `l65-demo-mid.png`). ⚠️ 백엔드 `start_time`·`/me` 변경은 로컬만 반영(재시작함) — pytest는 CI. ⛔ **교훈: 전체페이지 스샷으로 "맞췄다" 단정 금지, real·demo를 같은 폭 카드 단위로 대조.**
- ✅ **L66 [직원 관리] 목록 행 색 동그라미 제거 — 해소(2026-09-01, 커밋 dfcc80d)**: 「마지막 로그인」 옆 캘린더 색 원이 로그인 상태처럼 오인되고 편집도 못 하는 표시라 제거(사용자 지적). 색은 프로필 패널의 PalettePicker에서만 보이고 고친다. `CAL-COLOR-08`(비의사 색칸 생략) 확장 — 이제 목록엔 의사도 색 표시 없음. StaffList.tsx `colorCell`·`colorDot` 삭제. 헤드리스 admin 눈대조.
- ✅ **L67 [직원 관리] 캘린더 색 픽커 「검정 네모」 제거 — 해소(2026-09-01, 커밋 dfcc80d)**: 칩 안 strong 막대(띠, `band`)가 strong 색이 어두운 팔레트에선 검정 네모처럼 보여 헷갈린다는 지적. 막대를 빼고 **채움색 배경 + 「사용중」 표시만** 남김(데모 팔레트는 애초에 채움색 정사각형뿐). `CAL-COLOR-12`(면+띠) 조정 — 띠 제거, 면 색·글자색·사용중은 유지(테스트 3건 그대로 통과). 헤드리스 확인(선택=주황 채움 ring).
- ✅ **L68 [안내 보내기] 카드 좌우 스왑 — 해소(2026-09-01, 커밋 dfcc80d)**: 「보내는 내용」(좁은 폼)을 왼쪽, 「받는 사람 검색」(넓은 칸)을 오른쪽으로. SendPanel grid `minmax(300px,360px) minmax(0,1fr)` + DOM 순서 스왑. 헤드리스 확인.
- ✅ **L69 [오늘의 현황·장기 대기] 대기 분(分) 값이 두 번 나온다 — 해소(2026-09-01)**: 위치는 대기 목록(/queue)이 아니라 **오늘의 현황(/today)의 「장기 대기」 카드**였다(`Today.tsx:79`). 실제앱이 시각 레일을 `${wait_minutes}′`로 채워 **선두 `112′`와 우측 `112분 대기`가 같은 값**이 됐다. **근거 대조 결과 이건 사용자 결정이 필요한 취향 문제가 아니라 버그였다**: ①데모(`demo/src/routes/staff/mockData.ts:70`)는 장기 대기 행 선두를 **예약 시각**(`09:10`)으로, 우측을 `52분 대기`로 둔다(서로 다른 값·중복 없음) ②규칙 `TODAY-ROW-01`도 행 선두를 **「시각 레일」**로 명시. 즉 `112′` rail은 **TODAY-ROW-01 위반**이었다. → **rail을 예약 시각으로 되돌림**(다른 카드 미접수·전일미완료와 같은 축), 대기 분은 우측 사유 `N분 대기`로만(`TODAY-WAIT-01`). 당일 방문은 슬롯이 없어 rail에 「당일」. 배선: 백엔드 long_wait 쿼리에 `s.start_time as slot_time` 추가(이미 있던 `left join appointment_slots`), `LongWaitRow.slot_time` 타입 추가. 검증: vitest 신규 `[TODAY-ROW-01·L69]`(예약환자=시각·당일방문=「당일」·`42′` 없음) + 백엔드 `slot_time is not None` + 헤드리스(`now-fix-today.png`: 09:45/11:40/… 레일 + `129분 대기` 우측). ⚠️ 백엔드는 로컬 재시작 반영 — pytest는 CI.
- ✅ **L70 [대기 목록] 진료대기 상태 행 액션 버튼 가로 넘침(깨짐) — 해소(2026-09-01)**: 원인은 **실제앱이 데모에서 두 곳이 넓어진 것**: ①생년월일 열 `w-40`(160px, 데모는 `w-32`=128px) ②`환자 상세`에 붙인 `detail` 아이콘(+~23px, 2026-08-30 의도적 결정이라 유지). 전체 탭 진료대기 행(상태 배지 + 버튼 3개)에서 이 합이 `max-w-5xl` 카드를 넘겨 오른쪽 `환자 상세`가 잘렸다. → ①생년월일을 **데모대로 `w-32`로 복원**, ②진료과/의사 열을 **이 행에서 유일하게 양보하는 열**로(`min-w-0 truncate` + shrink 허용) — 붐비는 행에선 이 칸이 먼저 말줄임되고 버튼은 절대 안 잘린다. 검증: 헤드리스로 `max-w-5xl` 카드(cardW=1037) 기준 **모든 상태(진료대기·도착 포함 버튼 3개) `over: -14px`**(카드 안쪽 안착, 잘림 0). Queue.test 17 + tsc0.
- ✅ **L71 [의사 콘솔] 가운데 「환자 맥락」 열 좌우 회색 세로선이 두 줄로 보임 — 해소(2026-09-01)**: 사용자 지적("가운데 열 회색 세로 두 줄 맘에 안 든다, 데모도 이랬나"). 데모 대조 결과 **데모는 이렇지 않았다** — 데모는 열들이 `gap-0`에 각 열 `border-r`(얇은 1px) 하나씩(리사이저 없음). 실제앱은 데모에 없는 **드래그 열폭 조절 기능**(`DOCTOR-SHELL-04·05`, `ColumnResizer`)을 넣으며 **6px 회색 바**를 통째로 칠했고, 여기에 `contextCol`의 `borderRight`(1px)가 오른쪽 경계에서 바로 붙어 **회색선이 두 줄**로 보였다. → 기능은 유지하되 데모처럼 **얇은 단일선**으로: ①`ColumnResizer` 잡이 영역을 9px·**투명**으로 두고 그 안에 **가운데 1px 선**만 그림(가리키거나 끌 때만 딥틸로 굵어짐), ②`contextCol`의 중복 `borderRight` 제거(오른쪽 경계는 리사이저 선이 이미 긋는다). 검증: DoctorConsole 9 vitest·tsc0·헤드리스(`now-fix-console.png`: 좌·우 경계 모두 단일 얇은 선).
