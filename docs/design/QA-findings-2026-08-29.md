...
- ⏳ **L69 [대기 목록·장기 대기] 대기 분(分) 값이 두 번 나온다 — NEEDS-USER-DECISION**: 장기 대기 목록에서 행 **왼쪽 선두 숫자 `112'`**(프라임=분)와 **오른쪽 `112분 대기`**가 같은 값이라 중복으로 보인다(사용자 지적 2026-09-01). 사용자 초기 제안(「앞의 `112'`로 통일 + `'`→`분` + 주황색」)은 **본인이 곧 철회**("아 그러면 안된다") — 아마 선두 숫자 칸이 다른 큐 화면(대기목록)에선 예약시각(`09:00`)·경과분 등 다른 의미로도 쓰여 일괄 변경이 어긋나기 때문으로 추정. **다음 세션에 어떻게 해소할지 사용자와 결정 필요**(선두 숫자를 뺄지 / 오른쪽 라벨을 뺄지 / 둘의 의미를 구분할지). 관련 컴포넌트: 장기 대기 목록 렌더러(선두 `'` 숫자 + 우측 「N분 대기」).
- ⏳ **L70 [대기 목록] 진료대기 상태 행 액션 버튼 가로 넘침(깨짐) — TODO(수정 가능, 사용자가 나중으로 미룸)**: `진료 대기` 상태 행(예: 박소율·서지훈)은 버튼이 4개(`진료 대기`·`응급/주의 표시`·`되돌리기`·`환자 상세`)라 행 폭을 넘겨 오른쪽 `환자 상세`가 잘린다. 미도착·완료 등 버튼 적은 행은 멀쩡. → 액션 열 폭 확보/줄바꿈 허용/버튼 축약 중 택해 넘침 해소. QueuePage(대기 목록) 행 레이아웃.
