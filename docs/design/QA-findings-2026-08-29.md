...
- ✅ **L59 [의사 진료] 완료 후 무동작처럼 보임 — 해소(2026-08-31)**: 조사 결과 완료 흐름은 정상 — 완료하면 그 환자는 진료완료가 되어 대기 3상태(도착·대기·진료중)에서 빠지고, 가운데 기록엔 「진료 완료 · 시각」 표시+읽기전용+[수정] 버튼(recordMode=read_only_editable)이 이미 붙는다. 문제는 **환자가 어디로 갔는지 안 보여** "아무 일 안 일어난" 느낌. → **L60의 「오늘 완료」 구역이 해소**(완료 환자가 그 아래로 내려가 보인다). RecordPanel [수정]은 원래도 완료 직후 떴다(코드 확인).
