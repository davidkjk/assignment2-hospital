...
- 🔴 **L11 [캘린더] 의사 칩 선택 시 나머지 칩 사라짐 (근본원인 확정)**: `chipDoctors`가 **필터된 격자**에서 만들어짐(`CalendarPage.tsx:97-105`가 `model.doctors` 기반, 그 model은 `selectedDoctorIds`로 API 필터한 결과 `CalendarPage:88`). → 한 명 고르면 API가 그 사람만 반환 → 칩도 그 사람만 남아 **다른 의사 추가선택 불가(순환)**. 칩은 필터와 무관한 **전체 의사 카탈로그**(별도 쿼리)에서 와야 함.
...
- 🟠 **L16 [통계 드릴다운 명단] 시각이 UTC ISO 원본 노출**: `DrilldownModal.tsx:88` `const when = row.occurred_at ?? ...` — `formatHospitalDateTime` 안 거쳐 `2026-08-29T17:45:10.000550Z`(마이크로초·Z) 그대로 표시. **확정 버그.** 병원 시각 포맷 적용 필요.
...
- 🟠 **L16b [병합 되돌림 모달] 병합 시각 UTC 원본 노출**: `UndoConfirmDialog`도 병합시각을 `2026-08-29T17:45:10.000550Z`로 표시 — L16이 `DrilldownModal` 외 **여기도**. 시각 포맷 누락이 여러 컴포넌트 공통 → 전역 점검 대상.
