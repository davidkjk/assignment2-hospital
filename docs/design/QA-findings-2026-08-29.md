...
- 🟠 **L51 [시드 갭] `hospital_hours` 표가 시드에 없음**(2026-08-29 발견): `seed_demo.sql`·마이그레이션 어디에도 `hospital_hours`(요일별 운영시간) INSERT가 없어, **재시드하면 이 표가 0행**이 된다. 영향 = 상담봇 "지금 문 열렸나"(`is_open`이 읽는 유일한 곳, `00041`)·병원 운영시간 화면이 빈 채로 남음. ⚠️ **예약과는 별개** — 예약 시각 검증은 `appointment_service.py:328` `resolve_day`(의사 스케줄=`doctor_schedule_rules`, 시드에 있음)를 본다. 그래서 손검수 중 본 "진료 시간 밖" 예약 실패는 이 갭이 아니라 **환자 앱 병렬작업 중 의사 스케줄/예외 데이터 흔들림**이 원인(DB 안정 후 재시드로 해소). → **`seed_demo.sql`에 `hospital_hours` 7행(월~토 09~18·점심 12~13, 일 휴무) 추가**해 재시드마다 자동 채워지게.
