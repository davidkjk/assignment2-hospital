...
- ✅ **L65 [의사 콘솔] 가운데 「환자 맥락」 열이 데모와 어긋나 못생김 — 해소(2026-09-01, 사용자 요청)**: ① 기본정보 카드 밖에 상태가 「도착」 한 단어로 **떠 있어** 조회 오류처럼 보이던 것 → 카드 안 `StatusBadge`로. ② 「오늘 예약 이유」·「사전문진」에 **아이콘+볼드 제목**이 없어 데모 `Block`과 다르던 것 → 공용 `ConsoleCard`(아이콘+볼드·1px 경계·`--shadow-panel`·`--radius-card`)로 세 섹션 통일. ③ 문답을 좁은 열에 맞춰 세로 적층. + L62b·L63 동반. 커밋 · tsc 0 · doctor 55/55 · 헤드리스 real↔demo 대조(`real-mid-selected`↔`demo-mid-selected`).
