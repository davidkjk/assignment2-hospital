...
| `WEBBOOK-*` · `WEBCARD-DEPT/DOC/DATE/TARGET/WHY-*` · `BOOK-BOT-WIZARD-*` | 웹 상담봇 예약 대화 흐름(2026-09-07): 자연어 예약 의도→진료과 카드→의사→날짜→시간→(늦은 관문 로그인)→대상→방문이유→확인→신청. 서버 무상태(카드 payload가 선택값 누적·매 단계 재검증). 로그인 전 nav는 X-Anon-Token(`/cards/revalidate` 익명 분기). 채널 분기(결정 B): 웹=대화 내 예약, 앱=`open_booking_wizard`로 예약 마법사 인계. 하이브리드 ①=증상 대화 추천 시 예약 카드 연결. | 설계 2026-09-07, 결정 ①~⑤·B | 설계 `docs/superpowers/specs/2026-09-07-webchat-booking-flow-design.md`; 계획 `docs/superpowers/plans/2026-09-07-webchat-booking-flow.md`; 화면 `docs/design/screen-behaviors.md` 「C. 웹 예약 앞흐름」; 정본 `docs/design/chatbot-source-of-truth.md:89` |
