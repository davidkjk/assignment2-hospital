...
- **배정됨**: `00070` = `system_error_service_outage`(A2, 서비스 전체 장애 배지 `ERRADM-NOTI-02`). **다음 빈 = `00071`**. ⚠️ **A3(의사콘솔)은 무마이그로 끝났다** — `is_urgent_flag`가 `00005`부터 있어 새 칸 불필요(핸드오프의 "A3 urgent칸 마이그" 오판). `00071`은 A4 등 다음 실제 DB 칸 필요분에 쓴다.
