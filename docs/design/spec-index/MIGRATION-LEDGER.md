...
| `00060`– | **배포(5단계)** | `2026-07-27-deployment.md` |

**다음 빈 번호**: 환자앱 `00033`은 이미 직원웹 것 → 환자앱 밴드는 `00032`까지 **가득**. 직원웹은 `00052`(overflow, S18 `merge_questionnaire_count`)까지 찼고 다음 = `00053`(챗봇 시작 전까지). 새 마이그가 필요하면 **자기 트랙 밴드의 다음 빈 번호**를 쓰고, 밴드가 차면 이 문서를 갱신해 대역을 넓힌다. ⚠️ 직원웹이 `00052`를 가져가 **챗봇 밴드는 `00053`–`00059`로 +1 시프트**했다(아래 챗봇 표 개별 번호는 플랜 기준 → 실제는 +1로 읽는다).
...
### 챗봇 `00053`–`00059` (직원웹 overflow로 +1 시프트 — 아래 표 번호는 플랜 기준, 실제 +1)
| # | 파일 | 내용 |
|---|---|---|
