...
| 00087 | `security_move_questionnaire_authz` | ⭐ **보안 감사 F-02**(High, 2026-09-05) — `move_questionnaire_response`(00020) SECURITY DEFINER가 old/new의 account·for_patient 동일만 검사하고 호출자 소유·lineage 미검사 + authenticated 전원 grant라, 직원·관리자가 같은 환자 두 예약 ID로 임상 문진을 무관 예약으로 이동(IDOR)해 다른 진료과/의사에게 노출 가능하던 것. 함수 내부에 `coalesce(patient_owns())` caller 인증 + 진료과 lineage + 목적지 문진없음 검증 추가 → 직원·타환자·진료과간 이동 차단, 정상 재예약(환자 change_booking) 그대로. 되돌림 가능. ⚠️ 원격 미적용 — db push. 꼬리 번호 |
| 00088 | `security_marketing_consent_exclusion_status` | ⭐ **보안 감사 F-04**(Medium, 2026-09-05) — 광고(`kind=marketing`) 발송이 `ads_consent`를 무시하고 전체 활성환자에게 나가던 미구현 스텁(규칙 SEND-ADS-01의 의도 미구현). `resolve_recipients`가 marketing이면 `ads_consent=true`만 필터 + `dispatch`가 발송시점 재확인해 그새 철회건은 `delivery_status='제외'`로 로그 남기고 제외. 이 마이그는 `delivery_status`에 `'제외'` 허용값 추가. 거래성 알림 무영향. 되돌림 가능. ⚠️ 원격 미적용 — db push. 꼬리 번호 |
| 00089 | `security_revoke_patient_self_insert` | ⭐ **보안 감사 F-05 벡터2**(Medium, 2026-09-05) — `patients_can_register_self`·`patients_can_insert_family_members`(00017) authenticated 자가 INSERT 정책이 동의행 불변식 없이 Supabase Data API 직접 insert로 동의 0건 활성환자 생성을 허용하던 것. 두 정책 제거. ✅ 사전확인: 환자앱·webchat·직원웹 어디도 Data API로 `patients` 직접 insert 안 함(가입=백엔드 POST /patient, 가족추가=`add_family_member`, 둘 다 service-role) → 안전. 직원 등록 정책 유지. 되돌림 가능. ⚠️ 원격 미적용 — db push. 꼬리 번호 |
| 00090 | `security_seal_relink_family_rpc` | ⭐ **보안 감사 F-01**(High, 2026-09-05) — 직원이 사유·실행자 남기고 해제한 가족 연결을, 환자가 `relink_family_link_self`(00018) 직접 호출 또는 `add_family_member` 재등록으로 되살리고 감사 트리오(unlinked_at/by/reason)까지 지우던 것. `relink_family_link_self` EXECUTE 회수(앱 미사용) + `add_family_member`는 **자가해제(트리오 null)만 재활성**, 직원철회(`unlinked_by NOT NULL`)는 409 "이 가족은 지금 연결할 수 없습니다."(환자앱이 "더 필요하시면 병원에 문의" 덧붙임), 감사 트리오 append-only(재활성 UPDATE서 미변경). 자가해제 재연결은 그대로. 되돌림 가능. ⚠️ 원격 미적용 — db push. 꼬리 번호 |
