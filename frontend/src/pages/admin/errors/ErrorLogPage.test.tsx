...
describe('ErrorLogPage /admin/errors', () => {
  test('[ERRADM-HEAD-01] 제목과 설명이 화면의 목적을 넓히지 않는다', async () => {
    ...
    expect(screen.queryByText('오류가 발생한 시간과 기능을 확인합니다')).toBeNull()
    expect(screen.getByText('이 기록은 수정하거나 삭제할 수 없습니다')).toBeVisible()
    // 읽기전용 고지는 목적을 환자 데이터·발송 이력으로 넓히지 않는다.
    // (발송 이력 언급은 별도 규칙 ERRADM-NOTI-01의 이중기록 경계 노트가 갖는다 — 그건 정당하므로 이 노트로 범위를 좁힌다.)
    const readonlyNote = screen.getByText('이 기록은 수정하거나 삭제할 수 없습니다').closest<HTMLElement>('[role="note"]')!
    expect(within(readonlyNote).queryByText(/환자 데이터|발송 이력/)).toBeNull()
  })
