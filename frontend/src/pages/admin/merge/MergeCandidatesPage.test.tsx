...
test('[MERGE-SHELL-01][MERGE-SHELL-03] 관리자는 들어오고, 진입 자체는 열람 기록이 아니다', async () => {
  ...
  expect(await screen.findByRole('heading', { name: '중복 환자 후보' })).toBeVisible()
...
test('[MERGE-HEAD-01] 제목과 설명을 그린다', async () => {
  renderMerge()
  expect(await screen.findByRole('heading', { name: '중복 환자 후보' })).toBeVisible()
  expect(screen.getByText('같은 사람의 환자 기록이 나뉘었는지 확인하고 병합을 검토합니다')).toBeVisible()
