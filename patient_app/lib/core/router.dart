...
GoRouter buildAppRouter({String initialLocation = '/login'}) => GoRouter(
      ...
      routes: [
        ...
        // AI 상담 탭 — 이전 상담 목록(하단 탭 셸 안). 행을 누르면 그 방으로 복원 이동(CHAT-HISTORY-RESTORE-01).
        GoRoute(
            path: '/chat',
            builder: (c, s) => AppShell(
                body: ChatHistoryView(onOpen: (id) => c.go('/chat/room/$id')),
                bottomTabs: const MainTabs())), // NAV-HOME-11(4단계 · CHAT-HISTORY-LIST-01)
